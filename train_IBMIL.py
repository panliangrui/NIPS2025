...
class BagDataset(Dataset):
    ...
    def get_bag_feats(self, csv_file_df, args):
        ...
        # feats = joblib.load(feats_csv_path)
...
def testDTFD(args, test_df, classifier, dimReduction, attention, UClassifier, \
    ...
    if epoch == args.num_epochs - 1:
        ...
        with open(log_path, 'a+') as log_txt:
            log_txt.write('{:.2f},{:.2f},{:.2f},{:.2f} \n'.format(log_rep['macro avg']['precision'] * 100,
                                                                  log_rep['macro avg']['recall'] * 100, avg_score * 100,
                                                                  sum(auc_value) / len(auc_value) * 100))
...
def main():
    ...
    parser.add_argument('--dataset', default='xiangya2_TMB_2_1', type=str, help='Dataset folder name')
    ...
    parser.add_argument('--c_path', nargs='+',
                        default='./datasets_deconf/xiangya2_TMB_2_1/train_bag_cls_agnostic_feats_proto_8.npy', type=str,
                        help='directory to confounders') #'./datasets_deconf/STAS/train_bag_cls_agnostic_feats_proto_2.npy'
    ...
    if args.c_path:
        save_path = os.path.join('deconf',  str(args.dataset), datetime.date.today().strftime("%m%d%Y"),
                                 str(args.dataset) + '_' + str(args.model) + '_' + str(args.agg) + '_c_path')
    else:
        save_path = os.path.join('baseline_KRAS_2', datetime.date.today().strftime("%m%d%Y"),
    ...
    for fold in range(5):
        ...
        train_loader = DataLoader(trainset, 1, shuffle=True, num_workers=8)
        testset = BagDataset(test_path, args)
        test_loader = DataLoader(testset, 1, shuffle=False, num_workers=8)
        ...
        for epoch in range(1, args.num_epochs):
            ...
            train_loss_bag = trainDTFD(args, train_loader, DTFDclassifier, \
                                       DTFDdimReduction, DTFDattention, DTFDattCls, optimizer_adam0, optimizer_adam1, epoch,
                                       criterion, \
                                       log_path=log_path)
