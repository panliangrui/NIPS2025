...
class IATModel(nn.Module):
    def __init__(self, gate=True, dropout=0.25, k_sample=8, n_classes=2, instance_loss_fn=nn.CrossEntropyLoss(),
                 subtyping=False, feat_in=768, hidden_dims=[768, 1536, 512, 768]):
...
class HAGmodel(nn.Module):
    def __int__(self):
        super(HAGmodel, self).__int__()
        self.milnet0 =IATModel() #IATModel(gate=True, dropout=0.25, k_sample=8, n_classes=2, instance_loss_fn=nn.CrossEntropyLoss(), subtyping=False, feat_in=768, hidden_dims=[1024,1536,512,1024]).cuda()
        self.milnet1 = IATModel()#IATModel(gate=True, dropout=0.25, k_sample=8, n_classes=2, instance_loss_fn=nn.CrossEntropyLoss(),
                                 #subtyping=False, feat_in=768, hidden_dims=[1024, 1536, 512, 1024]).cuda()
        self.milnet2 = IATModel()#IATModel(gate=True, dropout=0.25, k_sample=8, n_classes=2, instance_loss_fn=nn.CrossEntropyLoss(),
                                     #subtyping=False, feat_in=768, hidden_dims=[1024, 1536, 512, 1024]).cuda()

    def forward(self, x, bag_label, instance_eval=True, a=1):
        if a == 0:
            bag_prediction, Y_prob, _, A_raw, results_dict = self.milnet0(x, label=bag_label, instance_eval=instance_eval)
        elif a == 1:
            bag_prediction, Y_prob, _, A_raw, results_dict = self.milnet0(x, label=bag_label, instance_eval=instance_eval)
        elif a == 2:
            bag_prediction, Y_prob, _, A_raw, results_dict = self.milnet0(x, label=bag_label, instance_eval=instance_eval)
        return bag_prediction, Y_prob, _, A_raw, results_dict
