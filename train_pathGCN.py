...
class BagDataset(Dataset):
    ...
    def get_bag_feats(self, csv_file_df, args):
        ...
        feats_csv_path = feats_csv_path.replace('pt_files', 'graph')
        feats_csv_path = feats_csv_path.replace('.pt', '.pkl')

        feats = joblib.load(feats_csv_path)#joblib.load(feats_csv_path)
...
def train(train_df, milnet, criterion, optimizer, args, log_path):
    ...
    for i, (bag_label, bag_feats) in enumerate(train_df):
        label = np.array([bag_label.numpy()])
        ...
        if args.model == 'PatchGCN':
            with autocast():
                bag_prediction, Y_prob, Y_hat, A_path = milnet(bag_feats)
...
def test(test_df, milnet, criterion, optimizer, args, log_path, epoch):
    ...
    test_predictions = test_predictions.astype(np.float64)
...
def main():
    ...
    parser.add_argument('--dataset', default='LUAD_ALK_2_1', type=str, help='Dataset folder name')
    # parser.add_argument('--datasets', default='xiangya2', type=str, help='Dataset folder name')
    parser.add_argument('--split', default=0.2, type=float, help='Training/Validation split [0.2]')
    parser.add_argument('--model', default='PatchGCN', type=str, help='MIL model [PatchGCN, DeepGraph]')
    ...
    if args.c_path:
        ...
        save_path = os.path.join('baseline_ALK_2', datetime.date.today().strftime("%m%d%Y"),
    ...
    if args.model == 'PatchGCN':
        import Models.PathGCN as pathgcn
        milnet = pathgcn.PatchGCN_Surv(n_classes=args.num_classes).cuda()
    ...
    for fold in range(5):
        ...
        print('lood ahead optimizer in PatchGCN....')
        ...
        for epoch in range(1, args.num_epochs):
            ...
            train_loss_bag = train(trainset, milnet, criterion, optimizer, args, log_path)  # iterate all bags
            print('epoch time:{}'.format(time.time() - start_time))
            test_loss_bag, avg_score, aucs, thresholds_optimal = test(testset, milnet, criterion, optimizer, args,
                                                                      log_path, epoch)
